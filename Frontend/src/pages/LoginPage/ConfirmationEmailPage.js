  ...
  constructor(){
    ...
    render() {
      ...
      let click;
        ...
        click = <Link to={`/login`} className="badge badge-pill badge-light" style={{color:'red',textDecoration:'underline'}}>Login</Link>
        tip = <div className="font-weight-light" style={{fontSize : '18px'}}>Congratulation, Your account is activated successfully, Please login</div>

      } else if(this.state.isValidToken === 'fail') {
        
        click = <a className="badge badge-pill badge-light" style={{color:'red',textDecoration:'underline'}} onClick = {() => {this.handleResendEmail()}}>Click for resend confirmation email :)</a>
        tip = <div className="font-weight-light">Sorry, you token has expired, please try to confirm your email again</div>;

      } else if(this.state.isValidToken === 'activated'){

        click = <Link to={`/login`} className="badge badge-pill badge-light" style={{color:'red',textDecoration:'underline'}}>Login</Link>
        tip = <div className="font-weight-light" style={{fontSize : '18px'}}>Woops, Your account is already activated, Please</div>;
